...
# No RUN --mount=type=cache for pnpm here: Railway BuildKit only allows cache IDs
# like id=s/<service-uuid>-<target> (see https://docs.railway.app/reference/dockerfiles).
...
# Default `--bind lan` so reverse proxies (Fly, Railway, etc.) can reach the listener.
